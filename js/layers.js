addLayer("s", {
		...
		if(inChallenge("diff", 12)) { player.s.buyables[11] = player.s.buyables[11].add(new Decimal(diff).mul(layers.diff.effect()))
		                              player.s.bigBrainBoredom = player.s.bigBrainBoredom.add(new Decimal(diff).mul(layers.diff.effect())) }
	    ...
	    if(inChallenge("diff", 21)) { player.s.lel = challengeCompletions("diff", 21)
		                              if (player.s.bigBoi == 1) {player.s.buyables[21] = new Decimal(player.s.buyables[21]).sub(new Decimal(10).mul(new Decimal(challengeCompletions("diff", 21)).add(1).min(5)))
									                             player.s.bigBoi = player.s.bigBoi.sub(1)}
									  if (player.s.bigBoi == 0) {player.s.bigBoi = player.s.bigBoi}
		}
		else {if(player.s.bigBoi == 0) player.s.buyables[21] = new Decimal(player.s.buyables[21]).add(new Decimal(10).mul(new Decimal(challengeCompletions("diff", 21)).add(1).min(5)))
			  if(player.s.lel < challengeCompletions("diff", 21) && challengeCompletions("diff", 21) < 5) { player.s.buyables[21] = new Decimal(player.s.buyables[21]).sub(10)
...
addLayer("ab", {
        ...
        midsection: [
		["display-text", function() {return "To be able to prestige, enter the \"The Endgamer\" challenge."}],
		"blank",
...
addLayer("diff", {
		...
		challenges: {
			...
			12: {
				name: "Dear god no...",
			    challengeDescription() { return "Predicted boredom is passively generated by diff, but it's effects are divided by themselves twice.<br/>Challenge completed: " + formatWhole(challengeCompletions("diff", 12)) + "/" + formatWhole(this.completionLimit) },
